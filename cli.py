...
6. source 选填、且只能从原文提取：原文明确写出报道方时才输出，且为对象 {"type","reporter","quote"} 三者必填——type 取 4 值枚举（官方与当事人 / 媒体与记者 / 爆料与泄密 / 民间与聚合），判据是报道方身份（谁说的），不看消息内容；reporter 是报道方名称；quote 是原文原句（一字不差）。引用不出原文原句 = 无来源 = 整个 source 字段省略。严禁臆造来源：不得根据上下文、常识或实体名推断报道方（例如把"曼联球员专访"臆想成"曼联官网"）。
